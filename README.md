...
- **Phase 2 — coven daemon integration.** `[FROZEN, MERGED]` — crate-side contracts landed (`audit.rs`, `staging.rs`); daemon-side call site merged to coven `main` via PR #382 (`feat/threads-gate-validator`). Beads `.14` (epic), `.20` (FREEZE), and `.19` (merge gate — resolved by flipping this repo public) closed with evidence.
