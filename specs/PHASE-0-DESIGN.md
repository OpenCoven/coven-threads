...
### 3.4.1 Applied-write records (`apply_audit`) — coven-threads#5

This section previously named only verdict-shaped events; it was silent on Gate-4 *applied writes* (the silence coven-threads#5 named). Recorded here because the design doc is authoritative (see `AGENTS.md`): the contract below is what landed in `coven-threads-core` and is restated, not redesigned.

Gate-4 applied Tier-2 writes are auditable in the same single store as verdicts: `AuditEventType::ApplyAudit` (stable tag `apply_audit`) records each applied Tier-2 write — target, resolved surface, tier, prev/next SHA-256, bytes written — alongside the verdict rows it completes. `coven-threads-core` owns the record shape and DDL (`audit.rs`: `WardAuditRecord::for_apply`); the daemon owns the connection and the appends (downstream: coven#414 appends after the applied disposition in `familiar_edits` and reads back via the observe pattern).

**Where the content hashes ride:** `diff_hash` carries `next_sha256` — the post-write content hash, consistent with verdict rows where `diff_hash` holds the proposal diff hash. The complementary `prev_sha256` and `bytes_written` ride in the nullable `detail` TEXT column as a compact JSON object `{"prev_sha256":"<64-lowercase-hex-or-null>","bytes_written":N}` (`prev_sha256` is `null` when the pre-write content is unknown), query-accessible via SQLite JSON functions. Dedicated typed hash columns were rejected: adding them would force another `ward_audit` table rebuild in every deployed store at the moment the variant ships. The JSON keys are normative constants in `audit.rs` (`APPLY_AUDIT_DETAIL_KEY_PREV`, `APPLY_AUDIT_DETAIL_KEY_BYTES`).

**Migration story (SQLite cannot `ALTER` a CHECK):** consumers do not hand-roll a rebuild. `coven-threads-core` exports `WARD_AUDIT_SCHEMA_STATE_SQL` — a table-local fingerprint query returning one of four stable tags (`missing`, `legacy_v013`, `current_v020`, `unknown`) — plus `WARD_AUDIT_SCHEMA_SQL` (atomic, self-guarding initialization of the exact `current_v020` shape) and `WARD_AUDIT_MIGRATION_V020_SQL` (a detail-preserving, guarded table rebuild for the exact `legacy_v013` shape). Daemon contract: classify first — initialize when `missing`, migrate only `legacy_v013`, continue on `current_v020`, and fail closed on `unknown`. Neither SQL statement mutates database-wide `PRAGMA user_version`.

**Version pin:** the apply-audit surface (`AuditEventType::ApplyAudit` plus `WardAuditRecord::detail`) expands public exhaustive APIs, so it ships on the `0.2.x` line (`coven-threads-core` v0.2.0) rather than as a `^0.1` patch; consumers pin `0.2`.
...
- **2026-08-30 (post-freeze documentation amendment, coven-threads#5)** — no design change. §3.4 previously named only verdict-shaped events and was silent on applied-write records; this documents the apply-audit contract that had already landed in `coven-threads-core` (PRs #15, #23; changelog `0.2.0`): `AuditEventType::ApplyAudit` / `apply_audit` tag, hash placement (`diff_hash` = `next_sha256`; `prev_sha256` + `bytes_written` in JSON `detail`), the exported schema-state/migration contract (`WARD_AUDIT_SCHEMA_STATE_SQL` / `WARD_AUDIT_SCHEMA_SQL` / `WARD_AUDIT_MIGRATION_V020_SQL`), and the `0.2.x` version pin. Closes the §3.4 silence on applied-write records vs verdicts that coven-threads#5 flagged.
